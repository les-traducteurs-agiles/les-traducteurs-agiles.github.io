...
En effet, lorsqu'il y a quelques mois, j'ai lu [l'article de Vasco Duarte](http://www.les-traducteurs-agiles.org/estimation/2015/09/13/les-story-consideres-comme-dangereux.html), je l'ai vu "pointer" le bout de son nez (aucun jeu de mots là-dessus) mais j'ai aussi débattu des mérites de l'estimation en story points faite dans le but de :
...
* **Les attentes sur la livraison de projets à périmètre "fixe" sont souvent (toujours ?) basées sur une estimation initiale du périmètre (supposé) et combien de temps cela prendra pour livrer ce périmètre (une nouvelle supposition) conduisent à des dysfonctionnements patents comme les marches forcées, une qualité médiocre, etc ...**
...
Si le budgest est fixé, alors il n'y aucune manière de "dépasser le budget" afin de livrer le périmètre fixé. Toutefois "dépasser le budget" est un terme habituellement utilisé lorsque l'on parle de projets qui ont échoués. Si votre budget est vraiment une contrainte alors vous livrerez seulement ce qui peut être livré. Les méthodes agiles veillent à que vous livrez la plus forte valeur au métier. 
...
* **Demander aux équipes d'estimer la durée de leur travail (ou combien de points elles livreront en un sprint ou en une livraison de version, ce qui revient au même) a des connotations de mesure de leur travail par une tierce partie (le responsable), créant un environnement de peur et de manipulation de chiffres pour refléter ce qui est désiré plutôt que ce qui est prévisible** 
...
Le PO doit probablement connaître le ROI d'une story lorsqu'il la présente à l'équipe pour être livré. Afin de calculer le ROI, il doit connaître combien elle va coûter pour d'être livrée (sa durée).
...
À partir de ce moment-là, une équipe estimerait l'item en utilisant les story points et ensuite le PO, armé de la vélocité de l'équipe, peut estimer le ROI de l'item. Mais sans les _story points_, comment cela peut il être accomplit ?
...
Si l'item ne peut être livré pendant le sprint, le PO peut simplement regarder combien de stories ont été tirées d'un epic et déterminer la probabilité d'être livrées au prochain sprint ou au sprint d'après, basé sur la quantité de stories que l'équipe délivre habituellement. Cela me conduit naturellement vers le sujet de comment nous mesurons la vélocité en l'absence de _story points_.
...
Désormais, j'entre fermement sur le territoire de Duarte. La réponse est que nous comptons le nombre de stories plutôt que d'accumuler les _story points_, éliminant ainsi le besoin d'estimer. Comme je l'ai mentionné précédemment, les équipes scindent les stories en plus petites ayant grosso modo la même taille, donc compter combien de stories sont livrées lors de chaque sprint donne ainsi une mesure de vélocité satisfaisante. Si l'équipe livre habituellement 5 stories avec zéro défaut et ensuite un sprint avec 6 ou 7 stories ayant zéro défaut, une amélioration a été faite (en écartant la variance, qui existe quelque soit l'unité de mesure que vous utilisez pour la vélocité).
...
Relecture : [Sylvain Fraïssé](http://www.les-traducteurs-agiles.org/traducteurs/)

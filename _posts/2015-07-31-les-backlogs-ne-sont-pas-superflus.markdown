---
layout: post
title:  "Les backlogs ne sont pas du superflu"
date:   2015-07-31 21:59:53
published: true
categories: 
- backlog
---
...
### Pour certaines personnes, les listes de courses à faire et les backlogs de produits sont superflus. Je suis en désaccord. Personnellement, je les vois comme des dépôts d'idées et d'alternatives.
...
Il n'y a rien de superflu dans le concept de backlog. Un backlog lui-même n'est pas superflu. Ce qui compte c'est comment les personnes l'utilisent.
...
Lorsqu'il s'agit des livres, j'ai une liste de souhaits de la taille d'une planète. Non seulement, j'aime lire ; j'aime également chercher des livres. Je regarde la liste des meilleurs livres, les listes du top 100, les récompenses annuels, les critiques et les notes, les nouvelles parutions hebdomadaires, et les bibliographies des livres dans les autres livres. Tous les livres que je trouve potentiellement intéressants rejoignent ma liste géante de souhaits.
...
Évidemment, je vais être incapable de lire tous ces livres. Une liste de souhaits est une liste de "pourrait-avoir". **Je compare ma liste de souhaits à un paysage**. Je trouve ça sympa d'imaginer vaguement un paysage entier à travers lequel je me fraye un chemin à travers ces montagnes et forêts de livres. Ne pas avoir une vue globale du terrain signifierait que chaque étape se ferait à l'aveugle.
...
> Évidemment, lire quelque fois une livre au hasard, comme prendre un détour inattendu, peut réserver une agréable surprise. Mais mon temps est un bien des plus précieux. Je veux l'utiliser en toute conscience de mes choix.
...
Lorsqu'il s'agit d'_écrire_ des livres, je suis connu pour travailler avec des [`checklists` énormes](http://noop.nl/2014/01/checklist-for-book-writers.html). Une liste de choses à faire est une liste de "doivent-avoir". Chaque livre est un projet différent, et par conséquent, chaque `checklist` est différente de la précédent. Mais mes `checklists` contiennent toujours des items comme "Commencer avec un histoire", "Mettre des blagues idiotes", "Ajouter des références bibliographiques" et "Vérifier les doubles espaces".
...
**La nature d'une `checklist` est l'opposé d'une liste de souhaits**. Avec une liste de souhaits, l'idée est de faire certains des items de la liste. Même si vous prenez seulement un seul élément de toute la liste, si l'item est le choix parfait, alors la liste de souhaits entière est vue comme un succès. Toutefois, avec une `checklist`, vous devez faire tous les items. Même si vous oubliez un seul item sur toute la liste, si l'oubli de cet item vous cause des ennuis, alors la `checklist` entière est vue comme un échec. 
...
Alors qu'est-ce qu'une liste de choses à faire alors ? Est-ce une liste de souhaits ou une `checklist` ? Et que sont donc les soit-disants backlogs de produits, qui sont de manière basique des listes de choses à faire pour les équipes de développements ? Sont-ils principalement des "pourraient-avoir" ou des "doivent-avoir" ?
...
À mon avis, avec votre backlog de produit, cela devrait être la même chose. **Vosu devriez considérer votre backlog comme une liste de souhaits**. C'est une vue globale du paysage de vos options. Esquisser le paysage vous permet de choisir le chemin le plus prometteur pour votre projet. Et il vous permet de vous rappeler des chemins alternatifs qui vous sont ouverts lorsque le chemin sur lequel vous êtes ne vous satisfait pas.

> Your supermarket shopping list counts as a checklist. Your product backlog counts as a wish list.

> Votre liste de courses de supermarché compte comme une `checklist`. Votre backlog de produit compte comme une liste de souhaits.
...
Il y a une autre raison importante de conserver les listes : **La science de la créativité suggère que les personnes les plus innovantes sont celles qui rassemblent les idées venant de plusieurs sources et qui les transforment en réservoirs d'inspiration**. Non seulement, j'ai des listes de livres ; j'ai aussi des listes (ou plutôt des collections non triées) de citations, d'articles, d'images, d'idées professionnelles à moitié travaillées, et tout un ensemble de bordel de projets ayants échoués précédemment ou non achevés.
...
Les recherches indiquent que **les personnes créatives se complaisent dans des environnements un peu bordélique**, où il y a juste assez de bruits de fonds pour permettre à leurs cerveaux de faire des connections `serendipitous` entre des idées n'ayant aucun rapport entre elles. Tout le sujet de l'innovation est de créer quelque chose de nouveau à partir de choses déjà existantes. Il n'y a pas beaucoup de chances que ça arrive si vous n'avez pas de matériaux brutes avec lesquels jouer.
...
Et parce que nos cerveaux ne sont pas optimisés pour la mémoire, il est utile d'avoir certains outils pour nous permettre de stocker des quantités illimités de bordel. Pour ceux qui connaissent [Getting Things Done](http://en.wikipedia.org/wiki/Getting_Things_Done) se rappelleront qu'il s'agit d'un élément central dans la méthode de David Allen sur la productivité : **sortez tous le foutoir de votre cerveau et mettez-le dans un dépôt**, que vous pourrez explorer pour chercher des idées quand vous le voudrez.
...
De manière répétée, j'ai entendu des gens dirent que les "backlogs sont superflus parce qu'ils servent d'inventaire".  
![backlogs-are-waste](http://noop.nl/wp-content/uploads/2015/05/backlogs-are-waste.png)
...
_(Note :  j'utilise cette image montrant des citations de Vasco Duarte et Paul Klipp seulement comme exemple. J'ai vu ce même type de commentaire plusieurs fois ces dernières années.)_
...
Ma liste de souhaits de livres n'est pas un inventaire. C'est une vue globale de mes options. Seuls les livres que j'ai achetés, non lus, et qui restent à dormir sur mes étagères pourraient qualifier d'inventaire, parce que j'y ai investi en les achetant. (Mais je les considère toujours comme des options et comme d'une belle décoration de mon séjour.)
...
De plus, dire aux développeurs et aux concepteurs de toujours bien tenir et nettoyer leurs listes d'idées à moitié germées est comme d'**imposer une politique de bureau rangés à des créatifs**. Cela dénote un point de vue mécanique de la productivité, et cela pourrait la chose la plus stupide que vous pourriez demander à des personnes desquelles on attend de l'innovation. Si elles n'ont pas le droit de garder des listes d'idées, comment voulez-vous qu'elles fassent des produits innovants ?

> Treat your backlog as a messy repository of half-baked ideas, instead of a sanitized production queue.

> Traitez votre backlog comme un dépôt bordélique d'idées à moitié germées, à la place d'une chaîne de production en environnement stérile.
...
Il n'y a rien de superflu dans le concept de backlog. Un backlog par lui-même n'est pas superflu. Ce qui compte c'est comment les personnes l'utilisent. Les créatifs ont besoin d'outils pour stocker leurs idées et se rappeler de leurs options. Il n'y a que des travailleurs en usines que l'on devrait s'attendre à avoir un inventaire de stock réduit et leurs machines-outils propres.

Telling people that backlogs are waste is not helping anyone to be more innovative.

Dire aux gens que les backlogs sont superflus n'aidera en rien les personnes à être plus innovantes.

_photo credit: (c) 2013 Barnyz, Creative Commons 2.0_
- See more at: http://noop.nl/2015/05/backlogs-are-not-waste.html#sthash.hm9XGgAH.dpuf
